...
| Image mode model | Two internal modes (`ImageMode = 'crop' \| 'fit'`) but **three user-facing pills** in the toolbar: Crop, Fit, 1:1. The 1:1 pill is `mode='fit'` + `fitLockNative=true`. The active-pill predicate (`ui/render.ts:isModePillActive`) is the single source of truth for matching dataset → state, reused by the no-op guard in `ui/bindings.ts`. All pill clicks route through `appController.setImageMode(mode, { fitLockNative })`, which dispatches the atomic `imageSetModePreset` so `render()` never sees a half-applied transition. |
| Crop region / fit offsets in source-pixel coordinates | `domain/geometry.ts:buildImageRenderPlan` — single unified `ImageRenderPlan` shape (`srcX/Y/W/H` + `fittedWidth/Height` + `offsetX/Y`). **Always reads the crop box** (`box ÷ displayScale`) for `srcX/Y/W/H`. Two branches: `'fit'` (covers fit and fit-locked-native) AR-preservingly places the cropped region with `fitAlign`. Sub-branch on `fitLockNative`: when true → scale forced to 1 (1:1 semantics; box clamp upstream guarantees `srcW ≤ targetW`, `srcH ≤ targetH` so `fitOffset` is ≥ 0, no clipping); when false → scale = `baseScale × fitSizePct/100` with optional `fitNoUpscale` cap (scale ≤ 1) and a 1-px snap-up at full fit. `'crop'` returns `fittedWidth=targetW, fittedHeight=targetH, offsetX=offsetY=0` (fills the device — box is always device-AR-locked, enforced in `applyGeometry`'s box recompute). |
| Crop-box constraint clamp | Two single-invariant siblings + a dispatcher: `domain/geometry.ts:clampBoxToSource` (fit-free-scale: source bounds + `min(8, source)` floor), `clampBoxToDevice` (fit-locked-native: source bounds + ≤ device dims + `min(8, …)` floor), and `clampBoxForMode(mode, fitLockNative, params)` which routes between them. Crop mode bypasses both — its box is fully recomputed from device AR + workScale in `applyGeometry`, not preserved across changes. The dispatcher is the single entry point — never add a `mode` flag *inside* either clamp (each encodes one invariant). Called from `ui/imageCropBridge.ts` (resize-handle drag) and `features/image/controller.ts:applyGeometry` (device/zoom changes). |
...
- **`runtime.displayScale` / `workScale` / `dispImgW` / `dispImgH` / `box{X,Y,W,H}`** are written **only** by `applyGeometry` (and zeroed by `unloadImage`). Drag handlers may move `boxX/boxY` through `setBoxPosition` and resize handlers may move `boxW/boxH` through `setBoxSize` — both scoped to crop-interaction gestures. Two blessed exceptions outside gestures: `toggleMirrorH/V` reflect the rect via `domain/geometry.ts:mirrorBoxRect` (display-axis reflection matching the button) through `setBoxPosition`, and `setRotation` rotates the rect through `setBoxSize`+`setBoxPosition` (via `domain/geometry.ts:rotateBoxRect`) — both immediately before `dispatchAndRetransform` re-runs `applyGeometry`, so the transformed position feeds the prev-center math and the box stays over the same source content. Note the button→flag mapping is view-space: at rotation 90°/270° the H button toggles `mirrorV` and vice versa, because the mirror flags apply in source space before rotation in `buildRotatedSource`. Don't write to these fields anywhere else.
...
| `src/domain/geometry.ts` | domain | Editor scales + unified render plan + crop-box clamp + rotated-dims/fit-pct helpers | `computeEditorGeometry`, `buildImageRenderPlan`, `clampBoxToSource`, `clampBoxToDevice`, `clampBoxForMode`, `getImageAnalysisRegion`, `fitOffset`, `rotatedSourceDims`, `computeMaxFitSizePct`, `rotateBoxRect`, `mirrorBoxRect` |
...
| Compute crop region in source pixels | `buildImageRenderPlan` (unified plan covering crop fill, fit AR-preserving with size + no-upscale, and 1:1 native placement) | `domain/geometry.ts` |
| Clamp a candidate crop box (fit mode — source bounds + min-8 floor) | `clampBoxToSource` | `domain/geometry.ts` |
| Clamp a candidate crop box (1:1 mode — ≤ device dims + min-8 floor) | `clampBoxToDevice` | `domain/geometry.ts` |
...
| Crop resize handle drag (AR unlocked) | sync, sub-frame | direction-aware delta → `clampBoxForMode` → `setBoxSize`/`setBoxPosition` → `applyCropBoxToDom`, `requestConvert` (rAF) | source-canvas redraw, pica, anything O(sourceW×sourceH) on the move loop |
| Mode-pill change (crop / fit / 1:1) | full rebuild | atomic `imageSetModePreset` dispatch → `resetEditor` → full applyGeometry + raster rebuild + `clampBoxForMode` re-clamp | — |
