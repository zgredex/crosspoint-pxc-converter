...
| Crop-box constraint clamp (no-upscale + source bound) | `domain/geometry.ts:clampCropBox` — pins each axis to `min(targetX, sourceX)` independently (strict AND, not OR). Prevents `cropFitScale > 1` and the degenerate one-axis-sliver preview that an OR rule allowed. Called by `applyGeometry` (device/zoom changes) and `cropInteraction` handle drag. |
