...
| Crop region / fit offsets in source-pixel coordinates | `domain/geometry.ts:buildImageRenderPlan` — single unified `ImageRenderPlan` shape (`srcX/Y/W/H` + `fittedWidth/Height` + `offsetX/Y`) covering all three image modes. **Always reads the crop box** (`box ÷ displayScale`) for `srcX/Y/W/H`. Branches on `mode`: `'crop'` returns `fittedWidth=targetW, fittedHeight=targetH, offsetX=offsetY=0` (fills the device — box is always device-AR-locked, enforced in `applyGeometry`'s box recompute); `'fit'` AR-preservingly fits the cropped region with `fitAlign` letterbox + an optional `fitNoUpscale` cap (scale ≤ 1) and a `fitSizePct`-driven shrink (10–100% of max-fit) + 1-px snap-up at full fit; `'one-to-one'` returns `fittedWidth=srcW, fittedHeight=srcH` for native pixel placement (no resampling). The 1:1 box is clamped to ≤ device dims by `clampBoxToDevice`, so `fitOffset` always returns ≥ 0 — no clipping path. |
| Crop-box constraint clamp | Two siblings, one per non-crop mode — pick the right one, never add a `mode` flag (each encodes one invariant): `domain/geometry.ts:clampBoxToSource` (fit: source bounds + `min(8, source)` floor), `clampBoxToDevice` (1:1: source bounds + ≤ device dims + `min(8, …)` floor). Crop mode bypasses these — its box is fully recomputed from device AR + workScale in `applyGeometry`, not preserved across changes. Dispatched in `ui/imageCropBridge.ts` (resize-handle drag) and `features/image/controller.ts:applyGeometry` (device/zoom changes) via a `clampBoxForMode` helper. |
...
| Mode change (crop ↔ fit ↔ 1:1) | full rebuild | `resetEditor` → full applyGeometry + raster rebuild + `clampBoxForMode` re-clamp | — |
...
- **Don't add a `mode` flag to box clamps.** Each clamp encodes a single invariant: `clampBoxToSource` = fit's lax source bounds; `clampBoxToDevice` = 1:1's source bounds + ≤ device dims. Crop mode bypasses both — its box is fully recomputed from device AR in `applyGeometry`, not preserved across changes. Dispatch by mode at the call site (see `clampBoxForMode` in `features/image/controller.ts` and the bridge dispatch in `ui/imageCropBridge.ts`); don't dilute any single clamp's contract by branching internally.
- **Don't fire the convert pipeline on no-op clicks.** Selector pills (device, image mode, dither mode, fit position, background, GB palette) and idempotent-result buttons (contrast reset, tone reset, auto-levels) must early-return when the click wouldn't change state — otherwise the user sees a preview flash and a worker round-trip for clicking the already-active option. Selectors compare `button.dataset.<key>` against the matching state field (the canonical dataset→state pairing lives in `ui/render.ts:44-49`'s active-class predicates — reuse it, don't reinvent it). Resets compare against `initialImageState.*`; tone reset must also check `!autoLevelsApplied` because the action clears that flag too. Auto-levels checks `state.image.autoLevelsApplied`. Re-applicable buttons (rotation, mirror, GB scale +/−, change/unload) and `change`-event toggles / `input`-event sliders are naturally guarded — no early-return needed there. All these guards live at the click-handler site in `ui/bindings.ts`, not inside `appController` setters, so the controllers stay callable for any future programmatic path that legitimately wants to re-run.
